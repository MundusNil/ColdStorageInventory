...
在 **第一次** `git add` / `git commit` / `git push` 之前，必须先完成变更分组并输出 **分组清单**（见下文第 2 步）。未完成分组清单就提交或推送，视为未按本流程执行。

满足下列 **任一** 条件时，**禁止** 单分支、单 PR 落地（除非用户在本轮对话中 **明确** 说「合并成一个 PR」）：

| 条件 | 说明 |
|------|------|
| 文件数 | 本次落主线范围内 **>20 个** 变更文件（含子模块内文件） |
| scope 数 | 触及 **≥2 个** 独立 scope（例如 UI 组件、`stores/`、`bridges/`、`services/`、配置/文档各算不同 scope） |
| 子模块大改 | Git 子模块（如 `openreel-editor`）内同时含 UI、store、bridge 等多层改动 |
| 会话多组 | 会话中已分 A/B/C 或多轮交付（如「第 N 轮汉化」），各轮默认独立成组 |

**不得** 用下列理由合并为一个 PR：

- 「同一功能」「同一主题」「同一汉化」「都在一个子模块里」
- 「改动是一起做的」「会话里连续完成的」
- 「拆 PR 太麻烦」「用户只说了落主线」

仅当用户 **明确** 写出「合并成一个 PR」「不要拆分支」时，才可单 PR；仍须在 PR 正文说明为何例外。
...
2. 变更分组与分支拆分（落主线前必做）。
   ...
   在 push 之前，先盘点本地已提交与未提交改动，按 **可独立合入、可独立回滚** 的原则分组。先对照上文 **硬门槛**：若触发禁止单 PR 的条件，必须先拆组再动手。
   ...
   | 子模块内多层 | OpenReel：`display-labels` 映射 vs 面板 UI vs `project-store`/`bridges` 文案 | **至少 2～3 个 PR**；父仓子模块指针 bump **单独一组** |
   ...
   **若改动已在单一分支上堆在一起**（常见误操作），在 push 前从最新 `origin/main` 重建分支，例如：
   ...
   拆完后向用户输出 **分组清单**（分支名 → 包含 commit / 文件 → 计划 PR 标题），**经用户扫一眼或默认继续后**，再进入步骤 3。多组之间无依赖时，合入顺序建议：`fix` → `feat` → `chore`；有依赖时按依赖链从前到后合入。

   **子模块 + 父仓示例**（OpenReel 汉化类工作）：

   ```text
   PR1（子模块）feat/editor-localize-display-labels    → display-labels + 映射消费方
   PR2（子模块）feat/editor-localize-editor-panels     → 各 inspector/panel UI 文案
   PR3（子模块）chore/editor-localize-user-messages    → store / bridge / service 用户可见文案
   PR4（父仓）  chore/video-factory-bump-openreel-*   → 仅子模块 gitlink（依赖 PR1–3 合入后）
   ```

   禁止把 PR1–3 打成一个「汉化」大包再让父仓只 bump 一次指针。
...
- **触发硬门槛（>20 文件 / ≥2 scope / 子模块多层）却准备单 PR**
...
- **>20 文件或 ≥2 scope → 禁止单 PR**（除非用户明确合并）
- 「同一主题 / 同一子模块」**不是** 单 PR 理由
...
- 百余个文件的「同一主题汉化」仍用 **一个** 分支、**一个** PR squash 合入（应拆映射 / UI / store·bridge 等）。
- 以「都在 openreel 子模块里」为由拒绝拆分。
